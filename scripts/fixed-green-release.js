...
(function () {
  ...
  d3.selectAll('.fixed-container').each(function () {
    ...
    function scrolled() {
      // prevent mac elastic scroll over-corrections
      var scrollPosition = Math.min($doc.height() - pageHeight, Math.max(0, window.pageYOffset));
      if (pageWidth < VIZ.breakWidth || VIZ.ios) {
        ...
        fixedLeft.style("top", scrollPosition > y1 ? y1 - scrollPosition + "px" : scrollPosition < y0 ? y0 - scrollPosition + "px" : null);
